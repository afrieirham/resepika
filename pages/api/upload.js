...
export default async (req, res) => {
  ...
  if (envError)
    return res
      .status(500)
      .json({ status: false, message: 'Fail to setup environment', error: envError })
  ...
  if (uploadError)
    return res
      .status(500)
      .json({ status: false, message: 'Fail to upload asset', error: uploadError })
  ...
  if (processError)
    return res
      .status(500)
      .json({ status: false, message: 'Fail to process asset', error: processError })
  ...
  if (recipeError)
    return res
      .status(500)
      .json({ status: false, message: 'Fail to add recipe', error: recipeError })
  ...
  if (getAssetError)
    return res
      .status(500)
      .json({ status: false, message: 'Fail to get asset', error: getAssetError })
  ...
  if (getRecipeError)
    return res
      .status(500)
      .json({ status: false, message: 'Fail to get recipe', error: getRecipeError })

  const [, assetPublishError] = await promiseFormatter(assetToPublish.publish())
  if (assetPublishError)
    return res
      .status(500)
      .json({ status: false, message: 'Fail to publish asset', error: assetPublishError })

  const [, recipePublishError] = await promiseFormatter(recipeToPublish.publish())
  if (recipePublishError)
    return res
      .status(500)
      .json({ status: false, message: 'Fail to publish asset', error: recipePublishError })
